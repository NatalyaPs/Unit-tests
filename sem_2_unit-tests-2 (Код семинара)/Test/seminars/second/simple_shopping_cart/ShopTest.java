//см решение в папке "код семинара"//см решение в папке "lesson02"
...
class ShopTest {
    ...
    // заводим переменные (выносим ссылки на уровень класса), ч.б. можно было использовать аннотацию @BeforeEach
    // и метод setUp() для внесения повторяющихся во всех методах-тестах строк
    // НО БЕЗ @BeforeEach НЕ НАДО ОБЪЯВЛЯТЬ ЭТИ ПОЛЯ !!! Т.К. БУДУТ СОХРАНЯТЬСЯ ЗНАЧЕНИЯ ИЗ ПРЕДЫДУЩИХ МЕТОДОВ, ЧТО ПРИВЕДЕТ К НЕТОЧНОСТИ
    Shop shop;
    Cart cart;

    // делаем повтор этих 2х строк "перед каждым" тестом - т.е ссылки создаются здесь
    // т.е. мы эти 2 строки вырезали из всех методов и объявили 1 раз их. и ВСЕГДА В НАЧАЛЕ КАЖДОГО МЕТОДА БУДЕМ ПОЛУЧАТЬ ЧИСТЫЙ ОБЪЕКТ
    @BeforeEach
    void setUp() {
        // Arrange (Подготовка)
